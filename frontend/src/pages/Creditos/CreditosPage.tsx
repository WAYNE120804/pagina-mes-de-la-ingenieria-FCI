...
const CreditosPage = () => {
  ...
  const loadData = async () => {
    const [creditosRes, variantesRes, clientesRes, defaultsRes] = await Promise.all([
      client.get(endpoints.creditos()),
      client.get(endpoints.productoVariantes()),
      client.get(endpoints.clientes()),
      client.get(endpoints.creditosDefaults()),
    ]);
  ...
  const saldoInicial = Math.max(0, subtotal - Number(form.pagoInicial || 0));
  ...
  const filteredCreditos = useMemo(() => {
    ...
    return creditos.filter((credito) => {
      if (estadoFilter && credito.estado !== estadoFilter) {
        return false;
      }
  ...
  const addVariant = (variant: any) => {
    ...
    if (totalQuantity > Number(variant.stockActual || 0)) {
      setModalError(`No puedes fiar mas de ${variant.stockActual} unidades de ${variant.producto?.nombre}.`);
      return;
    }
  ...
  const handleCreateSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let clienteIdForCredito = form.clienteId;

      if (!clienteIdForCredito) {
        if (
          showClientForm &&
          ...
          setSavingClient(false);
          clienteIdForCredito = createdClient.id;
        } else {
      ...
      if (Number(form.pagoInicial || 0) > subtotal) {
        throw new Error('El pago inicial no puede superar el total.');
      }

      const payload = {
        clienteId: clienteIdForCredito,
        ...
        observacionPago: form.observacionPago,
        observaciones: form.observaciones,
      };

      const { data } = await client.post(endpoints.creditos(), payload);
  ...
  const openPaymentModal = (credito: any) => {
    setPaymentTarget(credito);
    setPaymentForm({
      ...initialPaymentForm,
      valor: Number(credito.saldoPendiente || 0),
    });
    setModalError(null);
    setSuccess(null);
  };
  ...
  const handlePaymentSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await client.post(endpoints.creditoPagos(paymentTarget.id), paymentForm);
  ...
  const openCancelModal = (credito: any) => {
    setCancelTarget(credito);
  ...
  const handleCancelSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await client.post(endpoints.creditoCancelacion(cancelTarget.id), {
    ...
    },
    {
      ...
      render: (row: any) => (
        <div>
          ...
          <div className="text-xs text-slate-500">Pagado {formatCOP(row.totalPagado)} de {formatCOP(row.total)}</div>
        </div>
      ),
    },
    {
    ...
    <div>
      ...
      >
        ...
        <form onSubmit={handleCreateSubmit} className="space-y-6">
              ...
              <div className="mt-4 grid gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-2">
                ...
                <label className="text-sm">
                  ...
                  <input
                    ...
                    onChange={(event) => setClientForm((current) => ({ ...current, cedula: event.target.value }))}
                  />
                </label>
                <label className="text-sm">
          ...
          <section className="rounded-2xl border border-slate-200 p-4">
            <h3 className="mb-4 text-lg font-semibold text-slate-900">2. Productos entregados</h3>
          ...
          <section className="rounded-2xl border border-slate-200 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <MoneyInput
                ...
                onChange={(value) => setForm((current) => ({ ...current, pagoInicial: value }))}
              />
              <label className="text-sm">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-xs uppercase tracking-[0.08em] text-slate-500">Pagado</p>
                <p className="mt-2 text-xl font-semibold text-slate-900">{formatCOP(form.pagoInicial)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
      ...
      <FormModal
        ...
        description="La cuota entra a la caja diaria y actualiza el saldo pendiente."
      ...
      >
        ...
        <form onSubmit={handlePaymentSubmit} className="grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 md:col-span-2">
            ...
            <p className="mt-1 text-xl font-semibold text-slate-900">{formatCOP(paymentTarget?.saldoPendiente || 0)}</p>
          </div>
          <MoneyInput
          ...
          <label className="text-sm">
            ...
            <select
              ...
              onChange={(event) => setPaymentForm((current) => ({ ...current, metodoPago: event.target.value }))}
            >
              {paymentOptions.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
          ...
          <label className="text-sm">
            ...
            <input
              ...
              onChange={(event) => setPaymentForm((current) => ({ ...current, observacion: event.target.value }))}
            />
          </label>
          <div className="flex gap-3 md:col-span-2">
            <button
              ...
              className="rounded-md bg-slate-900 px-4 py-2 text-sm text-white disabled:opacity-60"
            >
              {saving ? 'Guardando...' : 'Registrar cuota'}
            </button>
            <button
              type="button"
              className="rounded-md border border-slate-300 px-4 py-2 text-sm"
